...
def _emit_added_entries(
    output: OutputBuilder,
    entries: list,
) -> None:
    output.add_blank_line("error")
    output.add_line(
        f"Found {len(entries)} high-confidence sorted definition(s) not in index:",
        section="error",
    )
    output.add_blank_line("error")
...
def _emit_orphaned_entries(
    output: OutputBuilder,
    entries: list,
    index_file_name: str,
) -> None:
    output.add_line(
        f"Found {len(entries)} orphaned entry/entries in index:",
        section="error",
    )
    output.add_blank_line("error")
    for entry in sorted(entries, key=lambda e: e.name):
        output.add_error_line(
            ValidationIssue.create(
                ...
                entry.line_number,
                entry.line_number,
                message=f"`{entry.name}` not found in any tech spec file",
                severity="error",
...
def _emit_moved_entries(
    ...
    output.add_line(
        f"Found {len(entries)} definition(s) in wrong section:",
        section="error",
    )
    output.add_blank_line("error")
    for entry in sorted(entries, key=lambda e: e.name):
        ...
        output.add_error_line(
            ValidationIssue.create(
                ...
                entry.line_number,
                entry.line_number,
                message=f"`{entry.name}` in '{current_path}'",
                severity="error",
...
def _emit_link_updates(
    output: OutputBuilder,
    entries: list,
    index_file_name: str,
) -> None:
    output.add_line(
        f"Found {len(entries)} entry/entries with incorrect links:",
        section="error",
    )
    output.add_blank_line("error")
    for entry in sorted(entries, key=lambda item: item.name):
        ...
        output.add_error_line(
            ValidationIssue.create(
                ...
                entry.line_number,
                entry.line_number,
                message=f"`{entry.name}`: {current_link}",
                severity="error",
...
def _emit_unresolved_entries(
    output: OutputBuilder,
    entries: list,
) -> None:
    output.add_line(
        (
            f"Found {len(entries)} definition(s) with low confidence "
            f"(< 75%) not in index:"
        ),
        section="error",
    )
...
def generate_report(
    ...
    if not has_issues and not output.verbose:
        return
    ...
    if added_entries:
        _emit_added_entries(output, added_entries)

    if orphaned_entries:
        _emit_orphaned_entries(output, orphaned_entries, index_file_name)

    if moved_entries:
        _emit_moved_entries(output, moved_entries, parsed_index, index_file_name)

    if link_updates:
        _emit_link_updates(output, link_updates, index_file_name)

    if unresolved_entries:
        _emit_unresolved_entries(output, unresolved_entries)
