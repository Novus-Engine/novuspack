...
```markdown
- Spec ID: `NP.DOMAIN.PATH` <a id="spec-np-domain-path"></a>
```
...
- Every `Algorithm` subsection must define a stable Algorithm anchor on its own line at the start of the section (immediately after the Algorithm heading).
  ...
  <a id="algo-np-core-package-addfile"></a>

  1. Validate filesystem path exists. <a id="algo-np-core-package-addfile-step-1"></a>
  ````
...
- Algorithm anchors must be derived from the Spec ID normalization rule:
  ...
  - for example `Spec ID: NP.CORE.Package.AddFile` => `algo-np-core-package-addfile`
- If an Algorithm subsection contains a numbered procedure, it may define step anchors to link requirements and feature files to a specific step.
  ...
  - `1. Validate filesystem path exists. <a id="algo-np-core-package-addfile-step-1"></a>`
  - `- Compare checksums. <a id="algo-np-core-package-addfile-step-2-3"></a>` (step 2, substep 3)
...
- `../tech_specs/api_core.md#spec-np-core-package-readfile`
...
- `NP.CORE.Package.ReadFile - ../tech_specs/api_core.md#spec-np-core-package-readfile`
...
- Tags should retain the `NP` prefix to avoid collisions.
...
The documentation pipeline runs via **`make docs-check`** (and in CI by [`docs-check.yml`](../../.github/workflows/docs-check.yml)).

Use Make targets only.
Do not invoke the Python scripts directly unless instructed.
...
`make docs-check PATHS="file1.md,dir1,file2.md"` runs only path-aware checks on the given files and directories.

The following are skipped when `PATHS` is set because they require a full-repo view:
...
Run `make docs-check PATHS=<path/to/file.md>` for the file you changed.
Fix any reported errors and re-run until the check passes.

### Make Targets Quick Reference

- `make docs-check [PATHS="..."] [VERBOSE=1] [OUTPUT="file.txt"] [CHECK_COVERAGE=1]`
- `make lint-markdown [PATHS="..."]`
- `make validate-links [PATHS="..."] [VERBOSE=1] [OUTPUT="file.txt"] [CHECK_COVERAGE=1]`
- `make validate-heading-numbering [PATHS="..."] [VERBOSE=1] [OUTPUT="file.txt"]`
- `make validate-go-code-blocks [PATHS="..."] [VERBOSE=1] [OUTPUT="file.txt"]`
- `make validate-go-defs-index [VERBOSE=1]`
- `make validate-req-references [VERBOSE=1]`
- `make audit-coverage`
- `make generate-anchor FILE='path/to/file.md'`
- `make generate-anchor LINE='path/to/file.md:NNN'`
...
- Run `make docs-check` and fix issues until it passes.
