...
Feature: Metadata: Metadata-Only Package Definition (Zero File Count)
  ...
  @REQ-META-069 @happy
  Scenario: Metadata-only package has special metadata files
    Given an open NovusPack package
    And a metadata-only package
    When package is examined
  ...
  @REQ-META-069 @happy
  Scenario: Metadata-only package has zero total size
    Given an open NovusPack package
    And a metadata-only package
    When package is examined
    Then TotalSize is 0
  ...
  Scenario: CreateMetadataOnlyPackage creates new metadata-only package
    ...
    And package is ready for metadata files
  ...
  @REQ-META-082 @error
  Scenario: ValidateMetadataOnlyPackage fails if no special files exist
    Given an open NovusPack package
    And package with no special metadata files
    When ValidateMetadataOnlyPackage is called
    Then structured validation error is returned
    And error indicates missing special metadata files
