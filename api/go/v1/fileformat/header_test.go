...
func TestPackageHeaderFieldTypes(t *testing.T) {
	...
	if header.Magic != NPKMagic {
		t.Errorf("Magic = 0x%X, want 0x%X", header.Magic, NPKMagic)
	}
...
func TestPackageHeaderMagicValidation(t *testing.T) {
	tests := []struct {
		...
		{"Valid magic", NPKMagic, FormatVersion, false},
...
// Specification: ../../docs/tech_specs/package_file_format.md Section 2.2 - Package Version Fields Specification
...
func TestNewPackageHeader(t *testing.T) {
	...
	if header.Magic != NPKMagic {
		t.Errorf("Magic = 0x%08X, want 0x%08X", header.Magic, NPKMagic)
	}
	if header.FormatVersion != FormatVersion {
		t.Errorf("FormatVersion = %d, want %d", header.FormatVersion, FormatVersion)
	}
