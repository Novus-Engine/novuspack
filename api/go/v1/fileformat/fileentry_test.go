	...
	tests := []struct {
	...
	}{
		{
			...
			FileEntry{
				FileID:       1,
				PathCount:    1,
				Paths:        []PathEntry{{PathLength: 8, Path: "test.txt"}},
				HashCount:    0,
				Hashes:       []HashEntry{},
		...
		},
		{
			...
			FileEntry{
				...
				HashCount: 1,
				Hashes: []HashEntry{
					{HashType: HashTypeSHA256, HashPurpose: HashPurposeContentVerification, HashLength: 32, HashData: make([]byte, 16)},
				},
			},
			true,
		},
		...
		},
		{
			...
			FileEntry{
				...
				HashCount: 1,
				Hashes: []HashEntry{
					{HashType: HashTypeSHA256, HashPurpose: HashPurposeContentVerification, HashLength: 32, HashData: make([]byte, 32)},
				},
			},
			false,
		},
...
// TestFileEntryWriteToFixedOnly verifies WriteTo with fixed section only
...
func TestFileEntryRoundTrip(t *testing.T) {
	entry := FileEntry{
		...
		Reserved:         0,
		Paths: []PathEntry{
			{PathLength: 8, Path: "test.txt", Mode: 0644},
			{PathLength: 9, Path: "test2.txt", Mode: 0755},
		},
		Hashes: []HashEntry{
			{HashType: HashTypeSHA256, HashPurpose: HashPurposeContentVerification, HashLength: 32, HashData: make([]byte, 32)},
			{HashType: HashTypeSHA512, HashPurpose: HashPurposeDeduplication, HashLength: 64, HashData: make([]byte, 64)},
		},
		OptionalData: []OptionalDataEntry{
			{DataType: OptionalDataTagsData, DataLength: 10, Data: make([]byte, 10)},
		},
	...
	tests := []struct {
		...
		},
		{
			...
			FileEntry{
				...
				Hashes: []HashEntry{
					{HashType: HashTypeSHA256, HashLength: 32, HashData: make([]byte, 32)},
				},
			},
			&failingWriter{maxBytes: 120},
			true,
			"failed to write",
		},
		{
	...
	tests := []struct {
		...
		},
		{
			...
			func() *bytes.Reader {
				...
				data := buf.Bytes()
				return bytes.NewReader(data[:64+46+3]) // Fixed + path + partial hash header
			},
			true,
		},
		{
			...
			func() *bytes.Reader {
				...
				// Truncate to remove part of the optional data
				data := buf.Bytes()
				return bytes.NewReader(data[:64+46+36+2]) // Fixed + path + hash + partial optional data header
			},
			true,
		},
		{
			...
			func() *bytes.Reader {
				...
				// Write padding to reach HashDataOffset
				padding := make([]byte, 100-46)
				buf.Write(padding)
				// Write hash
		...
		},
		{
			...
			func() *bytes.Reader {
				...
				_ = binary.Write(&buf, binary.LittleEndian, uint8(1))    // HashCount
				_ = binary.Write(&buf, binary.LittleEndian, uint32(200)) // HashDataOffset (requires skip)
				_ = binary.Write(&buf, binary.LittleEndian, uint16(36))  // HashDataLen
				_ = binary.Write(&buf, binary.LittleEndian, uint16(0))   // OptionalDataLen
				_ = binary.Write(&buf, binary.LittleEndian, uint32(0))   // OptionalDataOffset
				_ = binary.Write(&buf, binary.LittleEndian, uint32(0))   // Reserved
				// Write path (46 bytes)
				pathEntry := PathEntry{PathLength: 8, Path: "test.txt"}
				if _, err := pathEntry.WriteTo(&buf); err != nil {
					panic(err)
				}
				// Only write 50 bytes of padding (need 154 more, but only write 50)
				padding := make([]byte, 50)
				buf.Write(padding)
				// Truncate here - not enough data to reach hash
				return bytes.NewReader(buf.Bytes())
			},
			true,
		},
		...
		},
		{
			...
			func() *bytes.Reader {
				...
				_ = binary.Write(&buf, binary.LittleEndian, uint8(0))    // EncryptionType
				_ = binary.Write(&buf, binary.LittleEndian, uint8(1))    // HashCount
				_ = binary.Write(&buf, binary.LittleEndian, uint32(46))  // HashDataOffset
				_ = binary.Write(&buf, binary.LittleEndian, uint16(36))  // HashDataLen
				...
				// Write hash (36 bytes)
				hashEntry := HashEntry{HashType: HashTypeSHA256, HashLength: 32, HashData: make([]byte, 32)}
				if _, err := hashEntry.WriteTo(&buf); err != nil {
					panic(err)
				}
				// Only write 50 bytes of padding (need 218 more, but only write 50)
				padding := make([]byte, 50)
				buf.Write(padding)
