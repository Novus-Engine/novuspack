...
func RegisterCoreGenericPatterns(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^([A-Z][a-zA-Z0-9]+) is called with (?:pattern|file|entry|path|context|type|data|key|value|options|config|compression type|encryption type|signature|tag|predicate|validator|validators|item|items|checksum|size|level|count|limit|source|target|output|input|compressionType|encryptionType|signature data|new signature|generated signature data|invalid (?:data|type|encryption type|compression type|entry|path)|file source and encryption type|entry and new path|error checking|validators|compression type and level|file path and compression type|unsupported compression type|target path|output path|path in that directory|a valid path|an invalid or malformed path|a valid path in that directory|that path|a comment and valid path|an invalid path|AppID and valid path|cancelled context|Comment, VendorID, and AppID|comment, VendorID, AppID, permissions, and valid path|options|compressionType (\d+)|compressionType (\d+) \(LZMA\)|ZSTD compression|ZSTD compression type|compression type and config|cancelled context|configuration) (.+)$`, methodIsCalledWithTyped)
	...
	ctx.Step(`^(.+) value (\d+)x(\d+)$`, numericValueXYPattern)
	...
	ctx.Step(`^(.+) with (.+)$`, parameterPattern)
	...
	ctx.Step(`^security (.+)$`, securityOperationProperty)
	...
	ctx.Step(`^comment (.+)$`, commentOperationProperty)

	// Consolidated "structure" patterns - Phase 8
	ctx.Step(`^structure (.+)$`, structureOperationProperty)
	...
	ctx.Step(`^(compression type|error message|memory usage|chunk size|worker pool|key management|error context|error type|performance is|each entry|special file|signatures are|package contains|hash data|errors are|structured errors|error types|compressed package|directory metadata|compression ratio|comment is|file index|streaming is) (.+)$`, twoWordPattern)
	...
	ctx.Step(`^DirectoryEntry pointer references immediate directory$`, directoryEntryPointerReferencesImmediateDirectory)
	...
	ctx.Step(`^FilePathSource and MemorySource$`, filePathSourceAndMemorySource)
	ctx.Step(`^FileSource interface$`, fileSourceInterface)
	ctx.Step(`^FileSource providing content$`, fileSourceProvidingContent)
	ctx.Step(`^FileSource that fails during processing$`, fileSourceThatFailsDuringProcessing)
	...
	ctx.Step(`^GetDirectoryMetadata retrieves directory entries$`, getDirectoryMetadataRetrievesDirectoryEntries)
	...
	ctx.Step(`^InheritedTags property contains cached inherited tags$`, inheritedTagsPropertyContainsCachedInheritedTags)
	...
	ctx.Step(`^MemorySource has no filesystem overhead$`, memorySourceHasNoFilesystemOverhead)
	...
	ctx.Step(`^Mode stores directory permissions as octal$`, modeStoresDirectoryPermissionsAsOctal)
	...
	ctx.Step(`^RemoveFilePattern encounters error$`, removeFilePatternEncountersError)
	ctx.Step(`^RemoveFilePattern operation$`, removeFilePatternOperation)
	...
	ctx.Step(`^SecurityLevel reflects overall package security$`, securityLevelReflectsOverallPackageSecurity)
	ctx.Step(`^SecurityLevel reflects security state$`, securityLevelReflectsSecurityState)
	ctx.Step(`^SecurityLevel reflects validation state$`, securityLevelReflectsValidationState)
	ctx.Step(`^SecurityStatus\.SecurityLevel contains SecurityLevel value$`, securityStatusSecurityLevelContainsSecurityLevelValue)
	...
	ctx.Step(`^StoredSize reflects compressed size$`, storedSizeReflectsCompressedSize)
	ctx.Step(`^StreamConfig includes chunk size settings$`, streamConfigIncludesChunkSizeSettings)
	ctx.Step(`^strict UTF-(\d+) validation prevents encoding-based attacks$`, strictUTFValidationPreventsEncodingBasedAttacks)
	ctx.Step(`^system auto-detects optimal values$`, systemAutoDetectsOptimalValues)
	ctx.Step(`^TagValueTypeBoolean exists$`, tagValueTypeBooleanExists)
	ctx.Step(`^TagValueTypeFloat exists$`, tagValueTypeFloatExists)
	ctx.Step(`^TagValueTypeHash exists$`, tagValueTypeHashExists)
	ctx.Step(`^TagValueTypeInteger exists$`, tagValueTypeIntegerExists)
	ctx.Step(`^TagValueTypeJSON exists$`, tagValueTypeJSONExists)
	ctx.Step(`^TagValueTypeString exists$`, tagValueTypeStringExists)
	ctx.Step(`^TagValueTypeStringList exists$`, tagValueTypeStringListExists)
	ctx.Step(`^TagValueTypeTimestamp exists$`, tagValueTypeTimestampExists)
	ctx.Step(`^TagValueTypeUUID exists$`, tagValueTypeUUIDExists)
	ctx.Step(`^TagValueTypeVersion exists$`, tagValueTypeVersionExists)
	ctx.Step(`^TagValueTypeYAML exists$`, tagValueTypeYAMLExists)
	...
	ctx.Step(`^the Open method definition$`, theOpenMethodDefinition)
	...
	ctx.Step(`^ValidateDirectoryMetadata validates all directory metadata$`, validateDirectoryMetadataValidatesAllDirectoryMetadata)
	...
	// Additional catch-all patterns for remaining edge cases
	// Consolidated lowercase word + common verbs/connectors - Phase 11 Extended
	ctx.Step(`^([a-z][a-zA-Z0-9]+) (map|results|follows|uses|wrapping|via|without|with|for|in|at|on|to|from|by|as|into|onto|over|under|through|during|before|after|within|between|among|across|around|against|toward|towards|per) (.+)$`, lowercaseConnectorPattern)

	// Consolidated "X are Y" patterns (plural) - Phase 11 Extended
	ctx.Step(`^(.+) are (.+)$`, pluralPattern)
	...
	// Consolidated "X is Y" patterns (catch-all for remaining) - Phase 11 Extended
	ctx.Step(`^(.+) is (.+)$`, isPattern)

	// Consolidated "X, Y, Z fields exist" patterns - Phase 11 Extended
	ctx.Step(`^(.+), (.+), (.+) fields exist$`, multipleFieldsExistPattern)

	// Consolidated "X without Y" patterns - Phase 11 Extended
	ctx.Step(`^(.+) without (.+)$`, withoutPattern)

	// Consolidated "X wrapping Y" patterns - Phase 11 Extended
	ctx.Step(`^(.+) wrapping (.+)$`, wrappingPattern)

	// Consolidated "X from Y" patterns - Phase 11 Extended
	ctx.Step(`^(.+) from (.+)$`, fromPattern)

	// Consolidated "X to Y" patterns - Phase 11 Extended
	ctx.Step(`^(.+) to (.+)$`, toPattern)

	// Consolidated "X via Y" patterns - Phase 11 Extended
	ctx.Step(`^(.+) via (.+)$`, viaPattern)

	// Consolidated "X at Y" patterns - Phase 11 Extended
	ctx.Step(`^(.+) at (.+)$`, atPattern)

	// Consolidated "X on Y" patterns - Phase 11 Extended
	ctx.Step(`^(.+) on (.+)$`, onPattern)

	// Consolidated "X for Y" patterns - Phase 11 Extended
	ctx.Step(`^(.+) for (.+)$`, forPattern)

	// Consolidated "X in Y" patterns - Phase 11 Extended
	ctx.Step(`^(.+) in (.+)$`, inPattern)

	// Consolidated "X with Y" patterns (enhanced) - Phase 11 Extended
	ctx.Step(`^(.+) with (.+)$`, withPattern)

	// Consolidated "override" patterns - Phase 5
	ctx.Step(`^override (?:priority rules are (?:defined|followed))$`, overrideProperty)

	// Consolidated "oversized" patterns - Phase 5
	...
	// Consolidated Read patterns - Phase 5
	ctx.Step(`^Read (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, readProperty)

	// Consolidated read patterns - Phase 5
	ctx.Step(`^read (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, readLowercaseProperty)

	// Consolidated progress patterns - Phase 5
	ctx.Step(`^progress (?:is (?:examined|identified|retrieved|set|supported|used|tracked)|matches|supports (?:multiple algorithms|various key types))$`, progressProperty)

	// Consolidated Result patterns - Phase 5
	ctx.Step(`^Result (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types)|type is used)$`, resultProperty)
	...
	// Consolidated signing patterns - Phase 5
	ctx.Step(`^signing (?:is (?:examined|identified|retrieved|set|supported|used|performed)|matches|supports (?:multiple algorithms|various key types))$`, signingProperty)

	// Consolidated uncompressed patterns - Phase 5
	ctx.Step(`^uncompressed (?:data (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))|package(?:s)? (?:is|are) (?:examined|identified|retrieved|set|supported|used))$`, uncompressedProperty)

	// Consolidated SLH-DSA patterns - Phase 5
	ctx.Step(`^SLH-DSA (?:signature (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))|signatures (?:are (?:examined|identified|retrieved|set|supported|used)|approximately (\d+)-(\d+),(\d+) bytes))$`, slhDsaProperty)

	// Consolidated PackageCRC patterns - Phase 5
	ctx.Step(`^PackageCRC (?:is (?:examined|identified|retrieved|set|supported|used|calculated)|matches|supports (?:multiple algorithms|various key types))$`, packageCrcProperty)

	// Consolidated patterns patterns - Phase 5
	ctx.Step(`^patterns (?:are (?:examined|identified|retrieved|set|supported|used|demonstrated)|match|support (?:multiple algorithms|various key types))$`, patternsProperty)

	// Consolidated total patterns - Phase 5
	ctx.Step(`^total (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types)|size (?:is (?:examined|identified|retrieved|set|supported|used)|of all buffers is returned))$`, totalProperty)

	// Consolidated status patterns - Phase 5 (already registered above)

	// Consolidated valid patterns - Phase 5
	ctx.Step(`^valid (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types)|package(?:s)? (?:is|are) (?:examined|identified|retrieved|set|supported|used))$`, validProperty)

	// Consolidated SetMaxTotalSize patterns - Phase 5
	ctx.Step(`^SetMaxTotalSize is (?:called(?: (?:with max size)?)?|examined|identified|retrieved|set|supported|used)$`, setMaxTotalSizeProperty)

	// Consolidated public patterns - Phase 5
	ctx.Step(`^public (?:key(?:s)? (?:is|are) (?:examined|identified|retrieved|set|supported|used|provided)|matches|supports (?:multiple algorithms|various key types))$`, publicProperty)

	// Consolidated packages patterns - Phase 5
	ctx.Step(`^packages (?:are (?:examined|identified|retrieved|set|supported|used)|match|support (?:multiple algorithms|various key types))$`, packagesProperty)

	// Consolidated original patterns - Phase 5
	ctx.Step(`^original (?:file(?:s)? (?:is|are) (?:examined|identified|retrieved|set|supported|used|replaced atomically)|matches|supports (?:multiple algorithms|various key types))$`, originalProperty)

	// Consolidated verification patterns - Phase 5
	ctx.Step(`^verification (?:is (?:examined|identified|retrieved|set|supported|used|performed)|matches|supports (?:multiple algorithms|various key types))$`, verificationProperty)

	// Consolidated true patterns - Phase 5 (already registered above)

	// Consolidated TotalSize patterns - Phase 5
	ctx.Step(`^TotalSize is (?:called|examined|identified|retrieved|set|supported|used)$`, totalSizeIsProperty)
	ctx.Step(`^total size (?:is (?:examined|identified|retrieved|set|supported|used)|of all buffers is returned|reflects pool state)$`, totalSizeProperty)

	// Consolidated text patterns - Phase 5
	ctx.Step(`^text (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, textProperty)

	// Consolidated temp patterns - Phase 5
	ctx.Step(`^temp (?:file(?:s)? (?:is|are) (?:examined|identified|retrieved|set|supported|used|created|automatically cleaned up)|matches|supports (?:multiple algorithms|various key types))$`, tempProperty)

	// Consolidated Option patterns - Phase 5 (already registered in generics_steps.go)

	// Consolidated SignatureType patterns - Phase 5
	ctx.Step(`^SignatureType (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, signatureTypeProperty)

	// Consolidated returned patterns - Phase 5
	ctx.Step(`^returned (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, returnedProperty)

	// Consolidated ReadAt patterns - Phase 5
	ctx.Step(`^ReadAt (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, readAtProperty)
	...
	// Consolidated secure patterns - Phase 5
	ctx.Step(`^secure (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, secureProperty)

	// Consolidated script patterns - Phase 5 (already registered in file_types_steps.go)

	// Consolidated quantum-safe patterns - Phase 5
	ctx.Step(`^quantum-safe (?:encryption (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))|is (?:examined|identified|retrieved|set|supported|used))$`, quantumSafeProperty)

	// Consolidated processing patterns - Phase 5
	ctx.Step(`^processing (?:is (?:examined|identified|retrieved|set|supported|used|performed)|matches|supports (?:multiple algorithms|various key types))$`, processingProperty)

	// Consolidated paths patterns - Phase 5
	ctx.Step(`^paths (?:are (?:examined|identified|retrieved|set|supported|used)|match|support (?:multiple algorithms|various key types))$`, pathsProperty)

	// Consolidated PackageDataVersion patterns - Phase 5
	ctx.Step(`^PackageDataVersion (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, packageDataVersionProperty)

	// Consolidated workers patterns - Phase 5 (already registered in writing_steps.go)

	// Consolidated SignatureData patterns - Phase 5
	ctx.Step(`^SignatureData (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, signatureDataProperty)

	// Consolidated sentinel patterns - Phase 5
	ctx.Step(`^sentinel (?:error(?:s)? (?:is|are) (?:examined|identified|retrieved|set|supported|used|can be included)|matches|supports (?:multiple algorithms|various key types))$`, sentinelProperty)

	// Consolidated selection patterns - Phase 5
	ctx.Step(`^selection (?:is (?:examined|identified|retrieved|set|supported|used|performed)|matches|supports (?:multiple algorithms|various key types))$`, selectionProperty)
	...
	// Consolidated ReadHeader patterns - Phase 5
	ctx.Step(`^ReadHeader (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, readHeaderProperty)

	// Consolidated other patterns - Phase 5
	ctx.Step(`^other (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types)|signatures? (?:is|are) (?:not validated|examined|identified|retrieved|set|supported|used))$`, otherProperty)
	...
	// Consolidated Reserved patterns - Phase 5
	ctx.Step(`^Reserved (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types)|value(?:s)? (?:is|are) (?:examined|identified|retrieved|set|supported|used))$`, reservedCapitalProperty)

	// Consolidated parallel patterns - Phase 5
	ctx.Step(`^parallel (?:is (?:examined|identified|retrieved|set|supported|used|performed)|matches|supports (?:multiple algorithms|various key types)|processing (?:is (?:examined|identified|retrieved|set|supported|used|performed)))$`, parallelProperty)
	...
	// Consolidated values patterns - Phase 5
	ctx.Step(`^values (?:are (?:examined|identified|retrieved|set|supported|used)|match|support (?:multiple algorithms|various key types))$`, valuesProperty)

	// Consolidated unsupported patterns - Phase 5
	ctx.Step(`^unsupported (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types)|feature(?:s)? (?:is|are) (?:examined|identified|retrieved|set|supported|used))$`, unsupportedProperty)

	// Consolidated string patterns - Phase 5
	ctx.Step(`^string (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types)|value(?:s)? (?:is|are) (?:examined|identified|retrieved|set|supported|used))$`, stringProperty)

	// Consolidated Size patterns - Phase 5
	ctx.Step(`^Size (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, sizeProperty)

	// Consolidated signed patterns - Phase 5
	ctx.Step(`^signed (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types)|package(?:s)? (?:is|are) (?:examined|identified|retrieved|set|supported|used))$`, signedProperty)

	// Consolidated SHA-* patterns - Phase 5
	ctx.Step(`^SHA-(\d+) (?:hash (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))|is (?:examined|identified|retrieved|set|supported|used))$`, shaProperty)

	// Consolidated SecurityLevel patterns - Phase 5
	ctx.Step(`^SecurityLevel (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, securityLevelProperty)

	// Consolidated RemoveFilePattern patterns - Phase 5
	ctx.Step(`^RemoveFilePattern is (?:called(?: (?:with pattern)?)?|examined|identified|retrieved|set|supported|used)$`, removeFilePatternProperty)

	// Consolidated processed patterns - Phase 5
	ctx.Step(`^processed (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, processedProperty)

	// Consolidated overwrite patterns - Phase 5
	ctx.Step(`^overwrite (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, overwriteProperty)

	// Consolidated OriginalSize patterns - Phase 5
	ctx.Step(`^OriginalSize (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, originalSizeProperty)

	// Consolidated Validate patterns - Phase 5
	ctx.Step(`^Validate (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, validateProperty)

	// Consolidated Valid patterns - Phase 5
	ctx.Step(`^Valid (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, validCapitalProperty)

	// Consolidated UpdateFilePattern patterns - Phase 5
	ctx.Step(`^UpdateFilePattern is (?:called(?: (?:with pattern)?)?|examined|identified|retrieved|set|supported|used)$`, updateFilePatternProperty)

	// Consolidated texture patterns - Phase 5
	ctx.Step(`^texture (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, textureProperty)

	// Consolidated Tags patterns - Phase 5
	ctx.Step(`^Tags (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, tagsCapitalProperty)

	// Consolidated storage patterns - Phase 5 (already registered above)

	// Consolidated RemoveFile patterns - Phase 5
	ctx.Step(`^RemoveFile is (?:called(?: (?:with (?:path|pattern))?)?|examined|identified|retrieved|set|supported|used)$`, removeFileProperty)

	// Consolidated PublicKey patterns - Phase 5
	ctx.Step(`^PublicKey (?:is (?:examined|identified|retrieved|set|supported|used|provided)|matches|supports (?:multiple algorithms|various key types))$`, publicKeyProperty)

	// Consolidated Progress patterns - Phase 5
	ctx.Step(`^Progress (?:is (?:examined|identified|retrieved|set|supported|used|tracked)|matches|supports (?:multiple algorithms|various key types))$`, progressCapitalProperty)

	// Consolidated pattern patterns - Phase 5
	ctx.Step(`^pattern (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, patternProperty)
	...
	// Consolidated reader patterns - Phase 5
	ctx.Step(`^reader (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, readerProperty)
	...
	// Consolidated parameter patterns - Phase 5
	ctx.Step(`^parameter (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types)|s (?:are (?:examined|identified|retrieved|set|supported|used)))$`, parameterProperty)

	// Consolidated package-level patterns - Phase 5
	ctx.Step(`^package-level (?:compression (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))|is (?:examined|identified|retrieved|set|supported|used))$`, packageLevelProperty)

	// Consolidated optimal patterns - Phase 5 (already registered in compression_steps.go)

	// Consolidated video patterns - Phase 5
	ctx.Step(`^video (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, videoProperty)

	// Consolidated Version patterns - Phase 5
	ctx.Step(`^Version (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, versionCapitalProperty)

	// Consolidated variable-length patterns - Phase 5
	ctx.Step(`^variable-length (?:data (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))|is (?:examined|identified|retrieved|set|supported|used))$`, variableLengthProperty)

	// Consolidated usage patterns - Phase 5
	ctx.Step(`^usage (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, usageProperty)

	// Consolidated URL patterns - Phase 5
	ctx.Step(`^URL (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, urlProperty)

	// Consolidated TypedTag patterns - Phase 5
	ctx.Step(`^TypedTag (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, typedTagProperty)

	// Consolidated Timestamp patterns - Phase 5
	ctx.Step(`^Timestamp (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, timestampCapitalProperty)

	// Consolidated some patterns - Phase 5
	ctx.Step(`^some (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, someProperty)

	// Consolidated slice patterns - Phase 5
	ctx.Step(`^slice (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, sliceProperty)

	// Consolidated SignPackage patterns - Phase 5
	ctx.Step(`^SignPackage is (?:called(?: (?:with (?:key|options))?)?|examined|identified|retrieved|set|supported|used)$`, signPackageProperty)

	// Consolidated SignatureFlags patterns - Phase 5
	ctx.Step(`^SignatureFlags (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, signatureFlagsProperty)

	// Consolidated SecurityStatus patterns - Phase 5
	ctx.Step(`^SecurityStatus (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, securityStatusProperty)

	// Consolidated schema patterns - Phase 5 (already registered in metadata_steps.go)

	// Consolidated RemoveFilePath patterns - Phase 5
	ctx.Step(`^RemoveFilePath is (?:called(?: (?:with path)?)?|examined|identified|retrieved|set|supported|used)$`, removeFilePathProperty)

	// Consolidated RawChecksum patterns - Phase 5
	ctx.Step(`^RawChecksum (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, rawChecksumProperty)

	// Consolidated random patterns - Phase 5
	ctx.Step(`^random (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types)|access (?:is (?:examined|identified|retrieved|set|supported|used|enabled)))$`, randomProperty)

	// Consolidated process patterns - Phase 5 (already registered above)

	// Consolidated private patterns - Phase 5
	ctx.Step(`^private (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types)|key(?:s)? (?:is|are) (?:examined|identified|retrieved|set|supported|used|provided))$`, privateProperty)

	// Consolidated priority patterns - Phase 5
	ctx.Step(`^priority (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, priorityProperty)

	// Consolidated user patterns - Phase 5
	ctx.Step(`^user (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, userProperty)

	// Consolidated unified patterns - Phase 5
	ctx.Step(`^unified (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types)|file addition interface (?:is (?:examined|identified|retrieved|set|supported|used)))$`, unifiedProperty)

	// Consolidated underlying patterns - Phase 5
	ctx.Step(`^underlying (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, underlyingProperty)

	// Consolidated unchanged patterns - Phase 5
	ctx.Step(`^unchanged (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, unchangedProperty)

	// Consolidated Steam patterns - Phase 5 (already registered above)

	// Consolidated single patterns - Phase 5
	ctx.Step(`^single (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types)|step (?:is (?:examined|identified|retrieved|set|supported|used)))$`, singleProperty)

	// Consolidated SetVendorID patterns - Phase 5
	ctx.Step(`^SetVendorID is (?:called(?: (?:with (?:vendor ID|value))?)?|examined|identified|retrieved|set|supported|used)$`, setVendorIDProperty)

	// Consolidated Security patterns - Phase 5
	ctx.Step(`^Security (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, securityCapitalProperty)

	// Consolidated result patterns - Phase 5
	ctx.Step(`^result (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, resultLowercaseProperty)

	// Consolidated RemoveFileByPath patterns - Phase 5
	ctx.Step(`^RemoveFileByPath is (?:called(?: (?:with path)?)?|examined|identified|retrieved|set|supported|used)$`, removeFileByPathProperty)

	// Consolidated recovery patterns - Phase 5
	ctx.Step(`^recovery (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, recoveryProperty)
	...
	// Consolidated PackageInfo patterns - Phase 5
	ctx.Step(`^PackageInfo (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, packageInfoProperty)

	// Consolidated XXH* patterns - Phase 5 (already registered in signatures_steps.go)

	// Consolidated WithContext patterns - Phase 5
	ctx.Step(`^WithContext (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types)|helper function (?:is (?:examined|identified|retrieved|set|supported|used|available)))$`, withContextProperty)

	// Consolidated UpdateFileMetadata patterns - Phase 5
	ctx.Step(`^UpdateFileMetadata is (?:called(?: (?:with (?:path|metadata))?)?|examined|identified|retrieved|set|supported|used)$`, updateFileMetadataProperty)

	// Consolidated unencrypted patterns - Phase 5
	ctx.Step(`^unencrypted (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types)|data (?:is (?:examined|identified|retrieved|set|supported|used)))$`, unencryptedProperty)

	// Consolidated traditional patterns - Phase 5 (already registered above)

	// Consolidated target patterns - Phase 5
	ctx.Step(`^target (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types)|path(?:s)? (?:is|are) (?:examined|identified|retrieved|set|supported|used))$`, targetProperty)

	// Consolidated StreamingConfig patterns - Phase 5
	ctx.Step(`^StreamingConfig (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, streamingConfigCapitalProperty)

	// Consolidated StoredSize patterns - Phase 5
	ctx.Step(`^StoredSize (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, storedSizeProperty)

	// Consolidated SpecialFileInfo patterns - Phase 5
	ctx.Step(`^SpecialFileInfo (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, specialFileInfoProperty)

	// Consolidated SignatureSize patterns - Phase 5
	ctx.Step(`^SignatureSize (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, signatureSizeProperty)

	// Consolidated SignatureInfo patterns - Phase 5
	ctx.Step(`^SignatureInfo (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, signatureInfoProperty)

	// Consolidated SelectCompressionType patterns - Phase 5
	ctx.Step(`^SelectCompressionType is (?:called(?: (?:with (?:type|options))?)?|examined|identified|retrieved|set|supported|used)$`, selectCompressionTypeProperty)

	// Consolidated Seek patterns - Phase 5
	ctx.Step(`^Seek (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, seekProperty)
	...
	// Consolidated rules patterns - Phase 5
	ctx.Step(`^rules (?:are (?:examined|identified|retrieved|set|supported|used)|match|support (?:multiple algorithms|various key types))$`, rulesProperty)

	// Consolidated results patterns - Phase 5
	ctx.Step(`^results (?:are (?:examined|identified|retrieved|set|supported|used)|match|support (?:multiple algorithms|various key types))$`, resultsProperty)

	// Consolidated resistance patterns - Phase 5
	ctx.Step(`^resistance (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, resistanceProperty)

	// Consolidated requirements patterns - Phase 5
	ctx.Step(`^requirements (?:are (?:examined|identified|retrieved|set|supported|used)|match|support (?:multiple algorithms|various key types))$`, requirementsProperty)

	// Consolidated removed patterns - Phase 5
	ctx.Step(`^removed (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, removedProperty)

	// Consolidated recompression patterns - Phase 5
	ctx.Step(`^recompression (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, recompressionProperty)

	// Consolidated reason patterns - Phase 5
	ctx.Step(`^reason (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, reasonProperty)

	// Consolidated ReadFrom patterns - Phase 5
	ctx.Step(`^ReadFrom (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, readFromProperty)

	// Consolidated ReadFile patterns - Phase 5
	ctx.Step(`^ReadFile (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, readFileProperty)

	// Consolidated rawChecksum patterns - Phase 5
	ctx.Step(`^rawChecksum (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, rawChecksumLowercaseProperty)
	...
	// Consolidated WriteFile patterns - Phase 5 (already registered in writing_steps.go)

	// Consolidated volume patterns - Phase 5
	ctx.Step(`^volume (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, volumeProperty)

	// Consolidated violations patterns - Phase 5
	ctx.Step(`^violations (?:are (?:examined|identified|retrieved|set|supported|used)|match|support (?:multiple algorithms|various key types))$`, violationsProperty)

	// Consolidated various patterns - Phase 5
	ctx.Step(`^various (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, variousProperty)

	// Consolidated ValidationErrors patterns - Phase 5
	ctx.Step(`^ValidationErrors (?:are (?:examined|identified|retrieved|set|supported|used)|match|support (?:multiple algorithms|various key types))$`, validationErrorsCapitalProperty)

	// Consolidated UUID patterns - Phase 5
	ctx.Step(`^UUID (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, uuidProperty)

	// Consolidated Unicode patterns - Phase 5
	ctx.Step(`^Unicode (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, unicodeProperty)

	// Consolidated type-specific patterns - Phase 5
	ctx.Step(`^type-specific (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, typeSpecificProperty)

	// Consolidated type-based patterns - Phase 5
	ctx.Step(`^type-based (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, typeBasedProperty)

	// Consolidated Trusted patterns - Phase 5
	ctx.Step(`^Trusted (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types)|signatures? (?:is|are) (?:examined|identified|retrieved|set|supported|used))$`, trustedCapitalProperty)

	// Consolidated transparency patterns - Phase 5 (already registered above)

	// Consolidated trade-off patterns - Phase 5 (already registered above)

	// Consolidated time patterns - Phase 5
	ctx.Step(`^time (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, timeProperty)

	// Consolidated ThreadSafetyNone patterns - Phase 5
	ctx.Step(`^ThreadSafetyNone (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, threadSafetyNoneProperty)

	// Consolidated TempDir patterns - Phase 5
	ctx.Step(`^TempDir (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, tempDirProperty)

	// Consolidated tar-like patterns - Phase 5
	ctx.Step(`^tar-like (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, tarLikeProperty)

	// Consolidated tamper patterns - Phase 5
	ctx.Step(`^tamper (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types)|-proof (?:is (?:examined|identified|retrieved|set|supported|used)))$`, tamperProperty)

	// Consolidated tag-based patterns - Phase 5
	ctx.Step(`^tag-based (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, tagBasedProperty)

	// Consolidated structures patterns - Phase 5
	ctx.Step(`^structures (?:are (?:examined|identified|retrieved|set|supported|used)|match|support (?:multiple algorithms|various key types))$`, structuresProperty)

	// Consolidated strict patterns - Phase 5
	ctx.Step(`^strict (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, strictProperty)

	// Consolidated StoredChecksum patterns - Phase 5
	ctx.Step(`^StoredChecksum (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, storedChecksumProperty)

	// Consolidated spec patterns - Phase 5
	ctx.Step(`^spec (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, specProperty)

	// Consolidated space patterns - Phase 5
	ctx.Step(`^space (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, spaceProperty)

	// Consolidated sound patterns - Phase 5
	ctx.Step(`^sound (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, soundProperty)

	// Consolidated SignatureComment patterns - Phase 5
	ctx.Step(`^SignatureComment (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, signatureCommentProperty)

	// Consolidated SetPackageCompressionType patterns - Phase 5
	ctx.Step(`^SetPackageCompressionType is (?:called(?: (?:with (?:type|options))?)?|examined|identified|retrieved|set|supported|used)$`, setPackageCompressionTypeProperty)

	// Consolidated SetEncryptionKey patterns - Phase 5
	ctx.Step(`^SetEncryptionKey is (?:called(?: (?:with (?:key|options))?)?|examined|identified|retrieved|set|supported|used)$`, setEncryptionKeyProperty)

	// Consolidated SetAppID patterns - Phase 5
	ctx.Step(`^SetAppID is (?:called(?: (?:with (?:ID|value))?)?|examined|identified|retrieved|set|supported|used)$`, setAppIDProperty)

	// Consolidated second patterns - Phase 5
	ctx.Step(`^second (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, secondProperty)

	// Consolidated safe patterns - Phase 5
	ctx.Step(`^safe (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, safeProperty)

	// Consolidated runtime patterns - Phase 5
	ctx.Step(`^runtime (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, runtimeProperty)

	// Consolidated rule patterns - Phase 5
	ctx.Step(`^rule (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, ruleProperty)

	// Consolidated revocation patterns - Phase 5
	ctx.Step(`^revocation (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, revocationProperty)

	// Consolidated retry patterns - Phase 5
	ctx.Step(`^retry (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, retryProperty)

	// Consolidated relative patterns - Phase 5
	ctx.Step(`^relative (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, relativeProperty)

	// Consolidated references patterns - Phase 5
	ctx.Step(`^references (?:are (?:examined|identified|retrieved|set|supported|used)|match|support (?:multiple algorithms|various key types))$`, referencesProperty)

	// Consolidated reference patterns - Phase 5
	ctx.Step(`^reference (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, referenceProperty)

	// Consolidated readSpeed patterns - Phase 5
	ctx.Step(`^readSpeed (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, readSpeedProperty)

	// Consolidated reading patterns - Phase 5
	ctx.Step(`^reading (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, readingProperty)

	// Consolidated ReadChunk patterns - Phase 5
	ctx.Step(`^ReadChunk (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, readChunkProperty)

	// Consolidated raw patterns - Phase 5
	ctx.Step(`^raw (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, rawProperty)

	// Consolidated ProcessData patterns - Phase 5
	ctx.Step(`^ProcessData (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, processDataProperty)

	// Consolidated practices patterns - Phase 5
	ctx.Step(`^practices (?:are (?:examined|identified|retrieved|set|supported|used)|match|support (?:multiple algorithms|various key types))$`, practicesProperty)

	// Consolidated pattern-specific patterns - Phase 5
	ctx.Step(`^pattern-specific (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, patternSpecificProperty)

	// Consolidated Path patterns - Phase 5
	ctx.Step(`^Path (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, pathCapitalProperty)

	// Consolidated PackageBuilder patterns - Phase 5
	ctx.Step(`^PackageBuilder (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, packageBuilderProperty)

	// Consolidated oversized patterns - Phase 5
	ctx.Step(`^oversized (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, oversizedProperty)
	...
	// Consolidated WriteTo patterns - Phase 5
	ctx.Step(`^WriteTo (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, writeToProperty)

	// Consolidated WrapError patterns - Phase 5 (already registered above)

	// Consolidated WindowsAttrs patterns - Phase 5
	ctx.Step(`^WindowsAttrs (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, windowsAttrsProperty)

	// Consolidated very patterns - Phase 5
	ctx.Step(`^very (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, veryProperty)

	// Consolidated vendor patterns - Phase 5
	ctx.Step(`^vendor (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, vendorProperty)

	// Consolidated ValueType patterns - Phase 5
	ctx.Step(`^ValueType (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, valueTypeProperty)

	// Consolidated ValidateStreamingConfig patterns - Phase 5
	ctx.Step(`^ValidateStreamingConfig is (?:called(?: (?:with (?:config|options))?)?|examined|identified|retrieved|set|supported|used)$`, validateStreamingConfigProperty)

	// Consolidated ValidateSignatureIndex patterns - Phase 5
	ctx.Step(`^ValidateSignatureIndex is (?:called(?: (?:with (?:index|options))?)?|examined|identified|retrieved|set|supported|used)$`, validateSignatureIndexProperty)

	// Consolidated UTF-* patterns - Phase 5
	ctx.Step(`^UTF-(\d+) (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, utfEncodingProperty)

	// Consolidated users patterns - Phase 5
	ctx.Step(`^users (?:are (?:examined|identified|retrieved|set|supported|used)|match|support (?:multiple algorithms|various key types))$`, usersProperty)

	// Consolidated UnsupportedCompression patterns - Phase 5
	ctx.Step(`^UnsupportedCompression (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, unsupportedCompressionProperty)

	// Consolidated unsigned patterns - Phase 5
	ctx.Step(`^unsigned (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, unsignedProperty)

	// Consolidated UID patterns - Phase 5
	ctx.Step(`^UID (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, uidProperty)

	// Consolidated types patterns - Phase 5 (already registered above)

	// Consolidated trusted_source patterns - Phase 5
	ctx.Step(`^trusted_source (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, trustedSourceProperty)

	// Consolidated trusted patterns - Phase 5
	ctx.Step(`^trusted (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, trustedLowercaseProperty)

	// Consolidated ThreadSafetyReadOnly patterns - Phase 5
	ctx.Step(`^ThreadSafetyReadOnly (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, threadSafetyReadOnlyProperty)

	// Consolidated third patterns - Phase 5
	ctx.Step(`^third (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, thirdProperty)

	// Consolidated success patterns - Phase 5
	ctx.Step(`^success (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, successProperty)

	// Consolidated static patterns - Phase 5
	ctx.Step(`^static (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, staticProperty)

	// Consolidated source patterns - Phase 5
	ctx.Step(`^source (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, sourceProperty)

	// Consolidated smaller patterns - Phase 5
	ctx.Step(`^smaller (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, smallerProperty)

	// Consolidated small patterns - Phase 5
	ctx.Step(`^small (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, smallProperty)

	// Consolidated simple patterns - Phase 5
	ctx.Step(`^simple (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, simpleProperty)

	// Consolidated signature_type patterns - Phase 5
	ctx.Step(`^signature_type (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, signatureTypeLowercaseProperty)

	// Consolidated SignatureResults patterns - Phase 5
	ctx.Step(`^SignatureResults (?:are (?:examined|identified|retrieved|set|supported|used)|match|support (?:multiple algorithms|various key types))$`, signatureResultsProperty)

	// Consolidated SetFileTags patterns - Phase 5
	ctx.Step(`^SetFileTags is (?:called(?: (?:with (?:tags|options))?)?|examined|identified|retrieved|set|supported|used)$`, setFileTagsProperty)

	// Consolidated Set patterns - Phase 5
	ctx.Step(`^Set (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, setCapitalProperty)

	// Consolidated selected patterns - Phase 5
	ctx.Step(`^selected (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, selectedProperty)

	// Consolidated security_scan patterns - Phase 5
	ctx.Step(`^security_scan (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, securityScanProperty)

	// Consolidated SecurityLevel* patterns - Phase 5
	ctx.Step(`^SecurityLevel(?:None|Medium|Maximum|Low|High) (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, securityLevelProperty)

	// Consolidated search patterns - Phase 5
	ctx.Step(`^search (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, searchProperty)

	// Consolidated risk patterns - Phase 5
	ctx.Step(`^risk (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, riskProperty)

	// Consolidated returns patterns - Phase 5
	ctx.Step(`^returns (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, returnsProperty)

	// Consolidated resumable patterns - Phase 5
	ctx.Step(`^resumable (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, resumableProperty)

	// Consolidated relationship patterns - Phase 5
	ctx.Step(`^relationship (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, relationshipProperty)

	// Consolidated read-only patterns - Phase 5
	ctx.Step(`^read-only (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, readOnlyProperty)

	// Consolidated ratio patterns - Phase 5 (already registered above)

	// Consolidated priority-based patterns - Phase 5
	ctx.Step(`^priority-based (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, priorityBasedProperty)

	// Consolidated penetration patterns - Phase 5
	ctx.Step(`^penetration (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, penetrationProperty)

	// Consolidated PathLength patterns - Phase 5
	ctx.Step(`^PathLength (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, pathLengthProperty)

	// Consolidated ParseFileEntry patterns - Phase 5
	ctx.Step(`^ParseFileEntry (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, parseFileEntryProperty)

	// Consolidated PackageCompressionInfo patterns - Phase 5
	ctx.Step(`^PackageCompressionInfo (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, packageCompressionInfoProperty)

	// Consolidated originalSize patterns - Phase 5
	ctx.Step(`^originalSize (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, originalSizeLowercaseProperty)

	// Consolidated OptionalData patterns - Phase 5 (already registered above)

	// Consolidated ZSTD_compressStream* patterns - Phase 5
	ctx.Step(`^ZSTD_compressStream(\d+) (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, zstdCompressStreamProperty)

	// Consolidated Zstandard-specific patterns - Phase 5
	ctx.Step(`^Zstandard-specific (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, zstandardSpecificProperty)
	...
	// Consolidated WithTypedContext patterns - Phase 5
	ctx.Step(`^WithTypedContext (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, withTypedContextProperty)

	// Consolidated WithMetadata patterns - Phase 5
	ctx.Step(`^WithMetadata (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, withMetadataProperty)

	// Consolidated WithKeySize patterns - Phase 5
	ctx.Step(`^WithKeySize (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, withKeySizeProperty)

	// Consolidated Windows patterns - Phase 5
	ctx.Step(`^Windows (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, windowsProperty)

	// Consolidated ValueLength patterns - Phase 5
	ctx.Step(`^ValueLength (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, valueLengthProperty)

	// Consolidated Value patterns - Phase 5
	ctx.Step(`^Value (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, valueCapitalProperty)
	...
	// Consolidated ThreadSafetyConcurrent patterns - Phase 5
	ctx.Step(`^ThreadSafetyConcurrent (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, threadSafetyConcurrentProperty)

	// Consolidated thread-safe patterns - Phase 5
	ctx.Step(`^thread-safe (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, threadSafeProperty)

	// Consolidated this patterns - Phase 5
	ctx.Step(`^this (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, thisProperty)

	// Consolidated textures patterns - Phase 5
	ctx.Step(`^textures (?:are (?:examined|identified|retrieved|set|supported|used)|match|support (?:multiple algorithms|various key types))$`, texturesProperty)

	// Consolidated text-based patterns - Phase 5
	ctx.Step(`^text-based (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, textBasedProperty)

	// Consolidated test patterns - Phase 5
	ctx.Step(`^test (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, testProperty)

	// Consolidated supported patterns - Phase 5
	ctx.Step(`^supported (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, supportedProperty)

	// Consolidated suffix patterns - Phase 5
	ctx.Step(`^suffix (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, suffixProperty)

	// Consolidated sufficient patterns - Phase 5
	ctx.Step(`^sufficient (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, sufficientProperty)

	// Consolidated subsequent patterns - Phase 5
	ctx.Step(`^subsequent (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, subsequentProperty)

	// Consolidated StreamingConfigBuilder patterns - Phase 5
	ctx.Step(`^StreamingConfigBuilder (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, streamingConfigBuilderProperty)

	// Consolidated strategies patterns - Phase 5
	ctx.Step(`^strategies (?:are (?:examined|identified|retrieved|set|supported|used)|match|support (?:multiple algorithms|various key types))$`, strategiesProperty)

	// Consolidated Stop patterns - Phase 5
	ctx.Step(`^Stop (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, stopProperty)

	// Consolidated stats patterns - Phase 5
	ctx.Step(`^stats (?:are (?:examined|identified|retrieved|set|supported|used)|match|support (?:multiple algorithms|various key types))$`, statsProperty)

	// Consolidated Start patterns - Phase 5
	ctx.Step(`^Start (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, startProperty)

	// Consolidated SQL patterns - Phase 5
	ctx.Step(`^SQL (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, sqlProperty)
	...
	// Consolidated searchable patterns - Phase 5
	ctx.Step(`^searchable (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, searchableProperty)

	// Consolidated scripts patterns - Phase 5
	ctx.Step(`^scripts (?:are (?:examined|identified|retrieved|set|supported|used)|match|support (?:multiple algorithms|various key types))$`, scriptsProperty)
	...
	// Consolidated RSA patterns - Phase 5
	ctx.Step(`^RSA (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, rsaProperty)

	// Consolidated root patterns - Phase 5
	ctx.Step(`^root (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, rootProperty)

	// Consolidated revoked patterns - Phase 5
	ctx.Step(`^revoked (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, revokedProperty)

	// Consolidated reusable patterns - Phase 5
	ctx.Step(`^reusable (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, reusableProperty)

	// Consolidated re-signing patterns - Phase 5
	ctx.Step(`^re-signing (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, reSigningProperty)

	// Consolidated RemoveSpecialFile patterns - Phase 5
	ctx.Step(`^RemoveSpecialFile is (?:called(?: (?:with (?:file|path))?)?|examined|identified|retrieved|set|supported|used)$`, removeSpecialFileProperty)

	// Consolidated RemoveSignature patterns - Phase 5
	ctx.Step(`^RemoveSignature is (?:called(?: (?:with (?:signature|index))?)?|examined|identified|retrieved|set|supported|used)$`, removeSignatureProperty)

	// Consolidated RemoveMetadataFile patterns - Phase 5
	ctx.Step(`^RemoveMetadataFile is (?:called(?: (?:with (?:file|path))?)?|examined|identified|retrieved|set|supported|used)$`, removeMetadataFileProperty)

	// Consolidated removal patterns - Phase 5
	ctx.Step(`^removal (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, removalProperty)

	// Consolidated RefreshPackageInfo patterns - Phase 5
	ctx.Step(`^RefreshPackageInfo (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, refreshPackageInfoProperty)

	// Consolidated read-write patterns - Phase 5
	ctx.Step(`^read-write (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, readWriteProperty)

	// Consolidated range-based patterns - Phase 5
	ctx.Step(`^range-based (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, rangeBasedProperty)

	// Consolidated race patterns - Phase 5
	ctx.Step(`^race (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, raceProperty)

	// Consolidated ProcessingState patterns - Phase 5
	ctx.Step(`^ProcessingState (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, processingStateProperty)

	// Consolidated PreservePaths patterns - Phase 5
	ctx.Step(`^PreservePaths (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, preservePathsProperty)

	// Consolidated prefix patterns - Phase 5
	ctx.Step(`^prefix (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, prefixProperty)

	// Consolidated policy patterns - Phase 5
	ctx.Step(`^policy (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, policyProperty)

	// Consolidated per-path patterns - Phase 5
	ctx.Step(`^per-path (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, perPathProperty)

	// Consolidated Paths patterns - Phase 5
	ctx.Step(`^Paths (?:are (?:examined|identified|retrieved|set|supported|used)|match|support (?:multiple algorithms|various key types))$`, pathsCapitalProperty)

	// Consolidated parsing patterns - Phase 5
	ctx.Step(`^parsing (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, parsingProperty)

	// Consolidated override patterns - Phase 5 (already registered above)

	// Consolidated os patterns - Phase 5
	ctx.Step(`^os (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, osProperty)

	// Consolidated ZstandardStrategy patterns - Phase 5
	ctx.Step(`^ZstandardStrategy (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, zstandardStrategyProperty)

	// Consolidated writing patterns - Phase 5 (already registered in writing_steps.go)

	// Consolidated Writer patterns - Phase 5
	ctx.Step(`^Writer (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, writerCapitalProperty)

	// Consolidated WrapWithContext patterns - Phase 5
	ctx.Step(`^WrapWithContext (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, wrapWithContextProperty)

	// Consolidated wrapped patterns - Phase 5
	ctx.Step(`^wrapped (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, wrappedProperty)

	// Consolidated workChan patterns - Phase 5
	ctx.Step(`^workChan (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, workChanProperty)

	// Consolidated work patterns - Phase 5
	ctx.Step(`^work (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, workProperty)

	// Consolidated WithVendorID patterns - Phase 5
	ctx.Step(`^WithVendorID (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, withVendorIDProperty)

	// Consolidated WithTimestamp patterns - Phase 5
	ctx.Step(`^WithTimestamp (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, withTimestampProperty)

	// Consolidated WithSignatureType patterns - Phase 5
	ctx.Step(`^WithSignatureType (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, withSignatureTypeProperty)

	// Consolidated WithRandomIV patterns - Phase 5
	ctx.Step(`^WithRandomIV (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, withRandomIVProperty)

	// Consolidated WithEncryptionType patterns - Phase 5
	ctx.Step(`^WithEncryptionType (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, withEncryptionTypeProperty)

	// Consolidated WithEncryption patterns - Phase 5
	ctx.Step(`^WithEncryption (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, withEncryptionProperty)

	// Consolidated WithCompression patterns - Phase 5
	ctx.Step(`^WithCompression(?:,)? (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, withCompressionProperty)

	// Consolidated WithComment patterns - Phase 5
	ctx.Step(`^WithComment (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, withCommentProperty)

	// Consolidated WithAuthenticationTag patterns - Phase 5
	ctx.Step(`^WithAuthenticationTag (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, withAuthenticationTagProperty)

	// Consolidated WithAppID patterns - Phase 5
	ctx.Step(`^WithAppID (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, withAppIDProperty)

	// Consolidated Windows-specific patterns - Phase 5
	ctx.Step(`^Windows-specific (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, windowsSpecificProperty)

	// Consolidated WindowsAttributes patterns - Phase 5
	ctx.Step(`^WindowsAttributes (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, windowsAttributesProperty)

	// Consolidated whitespace-only patterns - Phase 5
	ctx.Step(`^whitespace-only (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, whitespaceOnlyProperty)

	// Consolidated when patterns - Phase 5
	ctx.Step(`^when (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, whenProperty)

	// Consolidated what patterns - Phase 5
	ctx.Step(`^what (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, whatProperty)

	// Consolidated warnings patterns - Phase 5
	ctx.Step(`^warnings (?:are (?:examined|identified|retrieved|set|supported|used)|match|support (?:multiple algorithms|various key types))$`, warningsProperty)

	// Consolidated vulnerabilities patterns - Phase 5
	ctx.Step(`^vulnerabilities (?:are (?:examined|identified|retrieved|set|supported|used)|match|support (?:multiple algorithms|various key types))$`, vulnerabilitiesProperty)

	// Consolidated vendor/platform patterns - Phase 5
	ctx.Step(`^vendor\/platform (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, vendorPlatformProperty)

	// Consolidated VendorIDInfo patterns - Phase 5
	ctx.Step(`^VendorIDInfo (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, vendorIDInfoProperty)

	// Consolidated vendor/application patterns - Phase 5
	ctx.Step(`^vendor\/application (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, vendorApplicationProperty)

	// Consolidated variable patterns - Phase 5
	ctx.Step(`^variable (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, variableProperty)

	// Consolidated Validator patterns - Phase 5
	ctx.Step(`^Validator (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, validatorCapitalProperty)

	// Consolidated ValidationRule patterns - Phase 5
	ctx.Step(`^ValidationRule (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, validationRuleProperty)

	// Consolidated ValidationError patterns - Phase 5
	ctx.Step(`^ValidationError (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, validationErrorProperty)

	// Consolidated Validation patterns - Phase 5
	ctx.Step(`^Validation (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, validationCapitalProperty)

	// Consolidated validating patterns - Phase 5
	ctx.Step(`^validating (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, validatingProperty)

	// Consolidated ValidateSpecialFiles patterns - Phase 5
	ctx.Step(`^ValidateSpecialFiles (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, validateSpecialFilesProperty)

	// Consolidated ValidateSignatureWithKey patterns - Phase 5
	ctx.Step(`^ValidateSignatureWithKey (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, validateSignatureWithKeyProperty)

	// Consolidated ValidateSignatureType patterns - Phase 5
	ctx.Step(`^ValidateSignatureType (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, validateSignatureTypeProperty)

	// Consolidated ValidateSignatureKey patterns - Phase 5
	ctx.Step(`^ValidateSignatureKey (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, validateSignatureKeyProperty)

	// Consolidated validateSignatureInternal patterns - Phase 5
	ctx.Step(`^validateSignatureInternal (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, validateSignatureInternalProperty)

	// Consolidated ValidateSignatureFormat patterns - Phase 5
	ctx.Step(`^ValidateSignatureFormat (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, validateSignatureFormatProperty)

	// Consolidated ValidateSignatureData patterns - Phase 5
	ctx.Step(`^ValidateSignatureData (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, validateSignatureDataProperty)

	// Consolidated ValidateSignatureComment patterns - Phase 5
	ctx.Step(`^ValidateSignatureComment (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, validateSignatureCommentProperty)

	// Consolidated ValidateMetadata patterns - Phase 5
	ctx.Step(`^ValidateMetadata (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, validateMetadataProperty)

	// Consolidated ValidateIntegrity patterns - Phase 5
	ctx.Step(`^ValidateIntegrity (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, validateIntegrityProperty)

	// Consolidated ValidateDirectoryMetadata patterns - Phase 5
	ctx.Step(`^ValidateDirectoryMetadata (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, validateDirectoryMetadataProperty)

	// Consolidated ValidateDecompressionData patterns - Phase 5
	ctx.Step(`^ValidateDecompressionData (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, validateDecompressionDataProperty)

	// Consolidated validated patterns - Phase 5
	ctx.Step(`^validated (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, validatedProperty)

	// Consolidated ValidateCompressionData patterns - Phase 5
	ctx.Step(`^ValidateCompressionData (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, validateCompressionDataProperty)

	// Consolidated ValidateCommentEncoding patterns - Phase 5
	ctx.Step(`^ValidateCommentEncoding (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, validateCommentEncodingProperty)

	// Consolidated ValidateComment patterns - Phase 5
	ctx.Step(`^ValidateComment (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, validateCommentProperty)

	// Consolidated ValidateAll patterns - Phase 5
	ctx.Step(`^ValidateAll (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, validateAllProperty)

	// Consolidated UseSolidCompression patterns - Phase 5
	ctx.Step(`^UseSolidCompression (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, useSolidCompressionProperty)

	// Consolidated user-selected patterns - Phase 5
	ctx.Step(`^user-selected (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, userSelectedProperty)

	// Consolidated UserID patterns - Phase 5
	ctx.Step(`^UserID (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, userIDProperty)

	// Consolidated user-friendly patterns - Phase 5
	ctx.Step(`^user-friendly (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, userFriendlyProperty)

	// Consolidated UseRandomIV patterns - Phase 5
	ctx.Step(`^UseRandomIV (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, useRandomIVProperty)

	// Consolidated UseParallelProcessing patterns - Phase 5
	ctx.Step(`^UseParallelProcessing (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, useParallelProcessingProperty)

	// Consolidated UseBufferPool patterns - Phase 5
	ctx.Step(`^UseBufferPool (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, useBufferPoolProperty)

	// Consolidated UpdateSignatureFile patterns - Phase 5
	ctx.Step(`^UpdateSignatureFile (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, updateSignatureFileProperty)

	// Consolidated UpdatePaths patterns - Phase 5
	ctx.Step(`^UpdatePaths (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, updatePathsProperty)

	// Consolidated UpdateMetadata patterns - Phase 5
	ctx.Step(`^UpdateMetadata (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, updateMetadataCapitalProperty)

	// Consolidated UpdateManifestFile patterns - Phase 5
	ctx.Step(`^UpdateManifestFile (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, updateManifestFileProperty)

	// Consolidated UpdateInheritedTags patterns - Phase 5
	ctx.Step(`^UpdateInheritedTags (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, updateInheritedTagsProperty)

	// Consolidated UpdateIndexFile patterns - Phase 5
	ctx.Step(`^UpdateIndexFile (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, updateIndexFileProperty)

	// Consolidated UpdateHashes patterns - Phase 5
	ctx.Step(`^UpdateHashes (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, updateHashesProperty)

	// Consolidated UpdateFileTags patterns - Phase 5
	ctx.Step(`^UpdateFileTags (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, updateFileTagsCapitalProperty)

	// Consolidated UpdateFileDirectoryAssociations patterns - Phase 5
	ctx.Step(`^UpdateFileDirectoryAssociations (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, updateFileDirectoryAssociationsCapitalProperty)

	// Consolidated updateFileDirectoryAssociations patterns - Phase 5
	ctx.Step(`^updateFileDirectoryAssociations (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, updateFileDirectoryAssociationsLowercaseProperty)

	// Consolidated UpdateDirectory patterns - Phase 5
	ctx.Step(`^UpdateDirectory (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, updateDirectoryProperty)

	// Consolidated Unwrap patterns - Phase 5
	ctx.Step(`^Unwrap (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, unwrapProperty)

	// Consolidated unused patterns - Phase 5
	ctx.Step(`^unused (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, unusedProperty)

	// Consolidated untrusted patterns - Phase 5
	ctx.Step(`^untrusted (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, untrustedProperty)

	// Consolidated UnsupportedErrorContext patterns - Phase 5
	ctx.Step(`^UnsupportedErrorContext (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, unsupportedErrorContextProperty)

	// Consolidated unspecified patterns - Phase 5
	ctx.Step(`^unspecified (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, unspecifiedProperty)

	// Consolidated UnsetEncryptionKey patterns - Phase 5
	ctx.Step(`^UnsetEncryptionKey (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, unsetEncryptionKeyProperty)

	// Consolidated unset patterns - Phase 5
	ctx.Step(`^unset (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, unsetProperty)

	// Consolidated unsaved patterns - Phase 5
	ctx.Step(`^unsaved (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, unsavedProperty)

	// Consolidated Unix patterns - Phase 5
	ctx.Step(`^Unix (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, unixProperty)

	// Consolidated Unified patterns - Phase 5
	ctx.Step(`^Unified (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, unifiedCapitalProperty)

	// Consolidated unauthorized patterns - Phase 5
	ctx.Step(`^unauthorized (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, unauthorizedProperty)

	// Consolidated typical patterns - Phase 5
	ctx.Step(`^typical (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, typicalProperty)

	// Consolidated Types patterns - Phase 5
	ctx.Step(`^Types (?:are (?:examined|identified|retrieved|set|supported|used)|match|support (?:multiple algorithms|various key types))$`, typesCapitalProperty)

	// Consolidated typed patterns - Phase 5
	ctx.Step(`^typed (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, typedProperty)

	// Consolidated two patterns - Phase 5
	ctx.Step(`^two (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, twoProperty)

	// Consolidated truncation patterns - Phase 5
	ctx.Step(`^truncation (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, truncationProperty)

	// Consolidated truncated patterns - Phase 5
	ctx.Step(`^truncated (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, truncatedProperty)

	// Consolidated transformed patterns - Phase 5
	ctx.Step(`^transformed (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, transformedProperty)

	// Consolidated tracking patterns - Phase 5 (already registered above)

	// Consolidated total_size patterns - Phase 5
	ctx.Step(`^total_size (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, totalSizeLowercaseProperty)

	// Consolidated totalBytes patterns - Phase 5
	ctx.Step(`^totalBytes (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, totalBytesProperty)

	// Consolidated ToSlice patterns - Phase 5
	ctx.Step(`^ToSlice (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, toSliceProperty)

	// Consolidated ToBinaryFormat patterns - Phase 5
	ctx.Step(`^ToBinaryFormat (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, toBinaryFormatProperty)
	...
	// Consolidated TagValueTypeStringList patterns - Phase 5
	ctx.Step(`^TagValueTypeStringList (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, tagValueTypeStringListProperty)

	// Consolidated TagValueTypeString patterns - Phase 5
	ctx.Step(`^TagValueTypeString (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, tagValueTypeStringProperty)

	// Consolidated TagValueTypeJSON patterns - Phase 5
	ctx.Step(`^TagValueTypeJSON (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, tagValueTypeJSONProperty)

	// Consolidated TagValueTypeInteger patterns - Phase 5
	ctx.Step(`^TagValueTypeInteger (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, tagValueTypeIntegerProperty)

	// Consolidated TagValueTypeHash patterns - Phase 5
	ctx.Step(`^TagValueTypeHash (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, tagValueTypeHashProperty)

	// Consolidated TagValueTypeFloat patterns - Phase 5
	ctx.Step(`^TagValueTypeFloat (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, tagValueTypeFloatProperty)

	// Consolidated TagValueTypeBoolean patterns - Phase 5
	ctx.Step(`^TagValueTypeBoolean (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, tagValueTypeBooleanProperty)

	// Consolidated tagType patterns - Phase 5
	ctx.Step(`^tagType (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, tagTypeProperty)

	// Consolidated TagsData patterns - Phase 5
	ctx.Step(`^TagsData (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, tagsDataProperty)

	// Consolidated TagCount patterns - Phase 5
	ctx.Step(`^TagCount (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, tagCountProperty)

	// Consolidated tabs patterns - Phase 5
	ctx.Step(`^tabs (?:are (?:examined|identified|retrieved|set|supported|used)|match|support (?:multiple algorithms|various key types))$`, tabsProperty)

	// Consolidated systems patterns - Phase 5
	ctx.Step(`^systems (?:are (?:examined|identified|retrieved|set|supported|used)|match|support (?:multiple algorithms|various key types))$`, systemsProperty)

	// Consolidated systematic patterns - Phase 5
	ctx.Step(`^systematic (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, systematicProperty)

	// Consolidated syntax patterns - Phase 5
	ctx.Step(`^syntax (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, syntaxProperty)

	// Consolidated symlink patterns - Phase 5
	ctx.Step(`^symlink (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, symlinkProperty)

	// Consolidated symbolic patterns - Phase 5
	ctx.Step(`^symbolic (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, symbolicProperty)

	// Consolidated switch patterns - Phase 5
	ctx.Step(`^switch (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, switchProperty)

	// Consolidated Sum patterns - Phase 5
	ctx.Step(`^Sum (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, sumProperty)

	// Consolidated successfully patterns - Phase 5
	ctx.Step(`^successfully (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, successfullyProperty)

	// Consolidated SubmitStreamingJob patterns - Phase 5
	ctx.Step(`^SubmitStreamingJob (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, submitStreamingJobProperty)

	// Consolidated SubmitJob patterns - Phase 5
	ctx.Step(`^SubmitJob (?:is (?:called|examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, submitJobProperty)

	// Consolidated Structure patterns - Phase 5 (already registered in file_format_steps.go)

	// Consolidated strong patterns - Phase 5
	ctx.Step(`^strong (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, strongProperty)

	// Consolidated StringList patterns - Phase 5
	ctx.Step(`^StringList (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, stringListProperty)

	// Consolidated StreamTimeout patterns - Phase 5
	ctx.Step(`^StreamTimeout (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, streamTimeoutProperty)

	// Consolidated StreamingWorkerPool patterns - Phase 5
	ctx.Step(`^StreamingWorkerPool (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, streamingWorkerPoolProperty)

	// Consolidated StreamingWorker patterns - Phase 5
	ctx.Step(`^StreamingWorker (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, streamingWorkerProperty)

	// Consolidated StreamingJob patterns - Phase 5
	ctx.Step(`^StreamingJob (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, streamingJobProperty)

	// Consolidated StreamBufferSize patterns - Phase 5
	ctx.Step(`^StreamBufferSize (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, streamBufferSizeProperty)

	// Consolidated Strategy patterns - Phase 5
	ctx.Step(`^Strategy (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, strategyCapitalProperty)

	// Consolidated stateless patterns - Phase 5
	ctx.Step(`^stateless (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, statelessProperty)

	// Consolidated standardized patterns - Phase 5
	ctx.Step(`^standardized (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, standardizedProperty)

	// Consolidated standard-compliant patterns - Phase 5
	ctx.Step(`^standard-compliant (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, standardCompliantProperty)

	// Consolidated stable patterns - Phase 5
	ctx.Step(`^stable (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, stableProperty)

	// Consolidated speed-critical patterns - Phase 5
	ctx.Step(`^speed-critical (?:is (?:examined|identified|retrieved|set|supported|used)|matches|supports (?:multiple algorithms|various key types))$`, speedCriticalProperty)

	// Consolidated specification patterns - Phase 5 (already registered above)

	// Consolidated SpecialFiles patterns - Phase 5
	ctx.Step(`^SpecialFiles (?:are (?:examined|identified|retrieved|set|supported|used)|match|support (?:multiple algorithms|various key types))$`, specialFilesProperty)
	...
	ctx.Step(`^WriteFile, RemoveFile, Write, SafeWrite, and FastWrite methods are available$`, writeFileRemoveFileWriteSafeWriteAndFastWriteMethodsAreAvailable)
	...
	ctx.Step(`^app ID demonstrates custom bit ID format$`, appIDDemonstratesCustomBitIDFormat)
	ctx.Step(`^app ID demonstrates custom format$`, appIDDemonstratesCustomFormat)
	ctx.Step(`^app ID enables application verification$`, appIDEnablesApplicationVerification)
	ctx.Step(`^app ID enables package association$`, appIDEnablesPackageAssociation)
	ctx.Step(`^app ID enables package identification$`, appIDEnablesPackageIdentification)
	ctx.Step(`^app ID equals$`, appIDEquals)
	ctx.Step(`^app ID equals or specified application identifier$`, appIDEqualsOrSpecifiedApplicationIdentifier)
	ctx.Step(`^app ID equals (\d+)x(\d+)$`, appIDEqualsX)
	ctx.Step(`^app ID equals (\d+)xABCDEF$`, appIDEqualsXABCDEF)
	ctx.Step(`^app ID equals (\d+)xB$`, appIDEqualsXB)
	ctx.Step(`^app ID equals (\d+)xDA$`, appIDEqualsXDA)
	ctx.Step(`^app ID examples are examined$`, appIDExamplesAreExamined)
	ctx.Step(`^app ID field contains application\/game identifier$`, appIDFieldContainsApplicationgameIdentifier)
	ctx.Step(`^app ID indicates no specific application association$`, appIDIndicatesNoSpecificApplicationAssociation)
	ctx.Step(`^app ID information is available$`, appIDInformationIsAvailable)
	ctx.Step(`^app ID is accessible via GetInfo$`, appIDIsAccessibleViaGetInfo)
	ctx.Step(`^app ID is an unsigned (\d+)-bit integer$`, appIDIsAnUnsignedBitInteger)
	ctx.Step(`^app ID is (\d+)-bit unsigned integer$`, appIDIsBitUnsignedInteger)
	ctx.Step(`^app ID is cleared$`, appIDIsCleared)
	ctx.Step(`^app ID is examined$`, appIDIsExamined)
	ctx.Step(`^app ID is included in package info$`, appIDIsIncludedInPackageInfo)
	ctx.Step(`^app ID is preserved correctly$`, appIDIsPreservedCorrectly)
	ctx.Step(`^app ID is set$`, appIDIsSet)
	ctx.Step(`^app ID is set in package header$`, appIDIsSetInPackageHeader)
	ctx.Step(`^app ID is set to$`, appIDIsSetTo)
	ctx.Step(`^app ID is set to any (\d+)-bit value$`, appIDIsSetToAnyBitValue)
	ctx.Step(`^app ID is set to no association or specified application identifier$`, appIDIsSetToNoAssociationOrSpecifiedApplicationIdentifier)
	ctx.Step(`^app ID is set to (\d+)x(\d+)$`, appIDIsSetToX)
	ctx.Step(`^app ID is set to (\d+)xABC$`, appIDIsSetToXABC)
	ctx.Step(`^app ID is set to (\d+)xABCDEF$`, appIDIsSetToXABCDEF)
	ctx.Step(`^app ID is set to (\d+)xB$`, appIDIsSetToXB)
	ctx.Step(`^app ID is set to (\d+)xDA$`, appIDIsSetToXDA)
	ctx.Step(`^app ID is set to (\d+)xFEDCBA$`, appIDIsSetToXFEDCBA)
	ctx.Step(`^app ID or VendorID operation is called$`, appIDOrVendorIDOperationIsCalled)
	ctx.Step(`^app ID provides package association$`, appIDProvidesPackageAssociation)
	ctx.Step(`^app ID represents Epic Games app$`, appIDRepresentsEpicGamesApp)
	ctx.Step(`^app ID represents Epic Games app ID$`, appIDRepresentsEpicGamesAppID)
	ctx.Step(`^app ID represents generic (\d+)-bit ID$`, appIDRepresentsGenericBitID)
	ctx.Step(`^app ID represents generic (\d+)-bit identifier$`, appIDRepresentsGenericBitIdentifier)
	ctx.Step(`^app ID represents Itch.io game ID$`, appIDRepresentsItchioGameID)
	ctx.Step(`^app ID represents Steam CS:GO$`, appIDRepresentsSteamCSGO)
	ctx.Step(`^app ID represents Steam TF$`, appIDRepresentsSteamTF)
	ctx.Step(`^app ID status is determined$`, appIDStatusIsDetermined)
	ctx.Step(`^app ID supports Epic Games Store identifiers$`, appIDSupportsEpicGamesStoreIdentifiers)
	ctx.Step(`^app ID supports numeric game IDs$`, appIDSupportsNumericGameIDs)
	ctx.Step(`^app ID supports proprietary systems$`, appIDSupportsProprietarySystems)
	ctx.Step(`^app ID value is returned$`, appIDValueIsReturned)
	ctx.Step(`^application identification is examined$`, applicationIdentificationIsExamined)
	ctx.Step(`^application identifier is cleared$`, applicationIdentifierIsCleared)
	ctx.Step(`^application identifier is set$`, applicationIdentifierIsSet)
	ctx.Step(`^application identifier usage is demonstrated$`, applicationIdentifierUsageIsDemonstrated)

	// Additional AppID/VendorID steps
	ctx.Step(`^AppID can be retrieved with GetAppID$`, appIDCanBeRetrievedWithGetAppIDSimple)
	ctx.Step(`^AppID is set to 0$`, appIDIsSetTo0)
	ctx.Step(`^AppID value is returned$`, appIDValueIsReturnedSimple)
	ctx.Step(`^check returns false if AppID is not set \(zero\)$`, checkReturnsFalseIfAppIDIsNotSetZero)
	ctx.Step(`^check returns false if VendorID is not set \(zero\)$`, checkReturnsFalseIfVendorIDIsNotSetZero)
	ctx.Step(`^check returns true if AppID is set \(non-zero\)$`, checkReturnsTrueIfAppIDIsSetNonZero)
	ctx.Step(`^check returns true if VendorID is set \(non-zero\)$`, checkReturnsTrueIfVendorIDIsSetNonZero)
	ctx.Step(`^ClearAppID is called$`, clearAppIDIsCalled)
	ctx.Step(`^ClearVendorID is called$`, clearVendorIDIsCalled)
	ctx.Step(`^current application identifier is retrieved$`, currentApplicationIdentifierIsRetrieved)
...
func anOpenNovusPackPackageWithAComment(ctx context.Context) error {
	// TODO: Create an open NovusPack package with a comment
	return godog.ErrPending
}
...
// Consolidated RemoveFilePattern pattern implementation - Phase 5

// removeFilePatternProperty handles "RemoveFilePattern is called...", etc.
func removeFilePatternProperty(ctx context.Context, property string) error {
	// TODO: Handle RemoveFilePattern property
	return godog.ErrPending
}
...
// Consolidated RemoveFile pattern implementation - Phase 5

// removeFileProperty handles "RemoveFile is called...", etc.
func removeFileProperty(ctx context.Context, property string) error {
	// TODO: Handle RemoveFile property
	return godog.ErrPending
}
...
func setVendorIDProperty(ctx context.Context, property string) error {
	// TODO: Handle SetVendorID property
	return godog.ErrPending
}
...
func setPackageIdentityProperty(ctx context.Context, property string) error {
	// TODO: Handle SetPackageIdentity property
	return godog.ErrPending
}
...
func removeFilePatternEncountersError(ctx context.Context) error {
	// TODO: Handle RemoveFilePattern encounters error
	return godog.ErrPending
}

func removeFilePatternOperation(ctx context.Context) error {
	// TODO: Handle RemoveFilePattern operation
	return godog.ErrPending
}
