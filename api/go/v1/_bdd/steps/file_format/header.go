...
func theHeaderIsParsed(ctx context.Context) (context.Context, error) {
	...
	err := header.Validate()
	if err != nil {
		// Store the error so it can be verified in subsequent steps
		// Don't return the error - let the test continue to verify the error was stored
		world.SetError(err)
		return ctx, nil
	}
	return ctx, nil
...
func theMagicFieldEquals0x4E56504B(ctx context.Context) error {
	...
	if header.Magic != novuspack.NPKMagic {
		return fmt.Errorf("magic field is 0x%08X, expected 0x%08X", header.Magic, novuspack.NPKMagic)
	}
	return nil
}
...
func headerIsInInitializedState(ctx context.Context) error {
	...
	if header.Magic != novuspack.NPKMagic {
		return fmt.Errorf("Magic = 0x%08X, want 0x%08X", header.Magic, novuspack.NPKMagic)
	}
...
func writtenDataIs112Bytes(ctx context.Context) error {
	...
	if len(buf) != 112 {
		return fmt.Errorf("written data is %d bytes, want 112", len(buf))
	}
	return nil
}
...
func headerFieldsMatchReaderData(ctx context.Context) error {
	...
	if header.Magic != novuspack.NPKMagic {
		return fmt.Errorf("Magic mismatch: %x != %x", header.Magic, novuspack.NPKMagic)
	}
	if header.FormatVersion != novuspack.FormatVersion {
