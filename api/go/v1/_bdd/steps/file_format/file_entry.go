...
func aFileEntryWithMultiplePathEntries(ctx context.Context) error {
	...
	entry := &novuspack.FileEntry{
		FileID: 1,
		Paths: []novuspack.PathEntry{
			{PathLength: 8, Path: "test.txt", Mode: 0644, UserID: 1000, GroupID: 1000},
			{PathLength: 12, Path: "test2.txt", Mode: 0755, UserID: 1000, GroupID: 1000},
			{PathLength: 10, Path: "test3.txt", Mode: 0600, UserID: 2000, GroupID: 2000},
		},
		HashCount:    0,
		Hashes:       []novuspack.HashEntry{},
		OptionalData: []novuspack.OptionalDataEntry{},
...
func allPathEntryFieldsArePreserved(ctx context.Context) error {
	...
	if readPathEntry.Mode != originalPathEntry.Mode {
		return fmt.Errorf("Mode not preserved: %d != %d", readPathEntry.Mode, originalPathEntry.Mode)
	}
	return nil
}
