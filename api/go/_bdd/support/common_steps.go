...
func anExistingPackageFileAt(ctx context.Context, path string) (context.Context, error) {
	...
	// Handle absolute paths like aFileAtPath does - create in temp dir to avoid permission issues
	var fullPath string
	if filepath.IsAbs(path) {
		// For absolute paths, create in temp dir with sanitized name
		baseName := filepath.Base(path)
...
func aFileThatIsNotAValidNovusPackPackage(ctx context.Context) (context.Context, error) {
	...
	path := world.TempPath("invalid-package.nvpk")
...
func anOpenPackageWithAttributes(ctx context.Context, state, locationOrAttribute string) error {
	...
	// getWorld already returns *World
	world := w
	...
	switch locationOrAttribute {
	case "with a comment", "with comment":
		...
		return nil
	case "with VendorID and AppID":
		err := pkg.SetPackageIdentity(11111, 22222)
		if err != nil {
			world.SetError(err)
			return err
		}
		return nil
	case "with AppID":
		err := pkg.SetAppID(12345)
		if err != nil {
			world.SetError(err)
			return err
		}
		return nil
	case "with VendorID", "with VendorID set":
		err := pkg.SetVendorID(67890)
		if err != nil {
			world.SetError(err)
			return err
		}
	...
	// For other attributes, return pending
	return godog.ErrPending
}
...
func anOpenPackageWithFiles(ctx context.Context, variation string) error {
	...
	// Handle "with comment" variation
	if variation == "with comment" {
		commentText := "test comment"
		err := pkg.SetComment(commentText)
		if err != nil {
			world.SetError(err)
	...
	// For other variations, return pending
	return godog.ErrPending
}
...
func createPlaceholderPackageFile(path string) error {
	...
	header := make([]byte, fileformat.PackageHeaderSize)
...
func errorIndicates(ctx context.Context, expectedText string) error {
	...
	switch expectedText {
	case "encoding issue":
		// Check for encoding/UTF-8 validation issues
		hasComment := strings.Contains(errMsgLower, "comment")
		hasEncodingIssue := hasComment && (strings.Contains(errMsgLower, "utf") ||
			strings.Contains(errMsgLower, "encoding") ||
			strings.Contains(errMsgLower, "invalid") ||
			strings.Contains(errMsgLower, "character") ||
			strings.Contains(errMsgLower, "not valid"))
		if !hasEncodingIssue {
			return fmt.Errorf("error message '%s' does not indicate encoding issue", errMsg)
		}
		return nil
	case "length limit exceeded":
		// Check for length limit exceeded issues
		hasComment := strings.Contains(errMsgLower, "comment")
		hasLength := strings.Contains(errMsgLower, "length")
		hasLengthIssue := hasComment && hasLength && (strings.Contains(errMsgLower, "exceed") ||
			strings.Contains(errMsgLower, "limit") ||
			strings.Contains(errMsgLower, "maximum") ||
			strings.Contains(errMsgLower, "max") ||
			strings.Contains(errMsgLower, "too long"))
		if !hasLengthIssue {
			return fmt.Errorf("error message '%s' does not indicate length limit exceeded", errMsg)
		}
		return nil
	case "corruption or I/O failure":
	...
	case "duplicate key":
		...
		errStr := strings.ToLower(errMsg)
		hasDuplicate := strings.Contains(errStr, "duplicate") ||
		...
		return nil
	case "tag not found":
		// Check for tag not found indication
		errStr := strings.ToLower(errMsg)
		hasTagNotFound := strings.Contains(errStr, "tag") && (strings.Contains(errStr, "does not exist") ||
		...
		return nil
	default:
		// Generic check for other error types
...
func errorTypeIs(ctx context.Context, expectedType string) error {
	...
	// Check if error is PackageError and verify type
	var pkgErr *novuspack.PackageError
	if errors.As(err, &pkgErr) {
		// Verify error type matches expected
		errTypeStr := pkgErr.Type.String()
		if !containsIgnoreCase(errTypeStr, expectedType) {
			return fmt.Errorf("error type %s does not match expected '%s'", errTypeStr, expectedType)
		}
		return nil
	}
	// Fallback: check if error message contains the type name
	errMsg := err.Error()
...
func structuredErrorIsReturned(ctx context.Context) error {
	...
	// Verify error is PackageError type
	var pkgErr *novuspack.PackageError
	if !errors.As(err, &pkgErr) {
...
func structuredValidationErrorIsReturned(ctx context.Context) error {
	...
	// Verify error is PackageError with ErrTypeValidation
	var pkgErr *novuspack.PackageError
	if !errors.As(err, &pkgErr) {
		return fmt.Errorf("error is not a PackageError: %T", err)
	}
	if pkgErr.Type != novuspack.ErrTypeValidation {
		return fmt.Errorf("error type is %s, expected Validation", pkgErr.Type.String())
	}
	return nil
}
...
func structuredIOErrorIsReturned(ctx context.Context) error {
	...
	// Verify error is PackageError with ErrTypeIO
	var pkgErr *novuspack.PackageError
	if !errors.As(err, &pkgErr) {
		return fmt.Errorf("error is not a PackageError: %T", err)
	}
	if pkgErr.Type != novuspack.ErrTypeIO {
		return fmt.Errorf("error type is %s, expected IO", pkgErr.Type.String())
	}
	return nil
}
...
func structuredContextErrorIsReturned(ctx context.Context) error {
	...
	// Verify error is PackageError with ErrTypeContext
	var pkgErr *novuspack.PackageError
	if !errors.As(err, &pkgErr) {
		return fmt.Errorf("error is not a PackageError: %T", err)
	}
	if pkgErr.Type != novuspack.ErrTypeContext {
		return fmt.Errorf("error type is %s, expected Context", pkgErr.Type.String())
	}
	return nil
}
