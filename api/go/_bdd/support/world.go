...
type World struct {
	...
	packages map[string]novuspack.Package // path -> package instance
	mu       sync.RWMutex

	// Current package context (for scenarios that work with a single package)
	currentPackage novuspack.Package
	currentPath    string
	...
	packageMetadata map[string]any // key -> value
	...
	// FileInfo storage for testing
	fileInfoList []novuspack.FileInfo
}
...
func NewWorld() (*World, error) {
	...
	return &World{
		WorkDir:         workDir,
		TempDir:         tempDir,
		packages:        make(map[string]novuspack.Package),
		files:           make(map[string][]byte),
		packageMetadata: make(map[string]any),
	}, nil
}
...
func (w *World) SetPackage(pkg novuspack.Package, path string) {
	w.mu.Lock()
	defer w.mu.Unlock()
...
func (w *World) GetPackage() novuspack.Package {
	w.mu.RLock()
	defer w.mu.RUnlock()
...
func (w *World) GetPackageByPath(path string) novuspack.Package {
	w.mu.RLock()
	defer w.mu.RUnlock()
...
func (w *World) Cleanup() error {
	...
	w.packageMetadata = make(map[string]any)
	w.testContext = nil
	w.testCancel = nil
	w.currentHeader = nil
...
func (w *World) SetPackageMetadata(key string, value any) {
	w.mu.Lock()
	defer w.mu.Unlock()
...
func (w *World) GetPackageMetadata(key string) (any, bool) {
	w.mu.RLock()
	defer w.mu.RUnlock()
	if w.packageMetadata == nil {
		return nil, false
...
// SetDirectoryEntry stores a DirectoryEntry for testing
func (w *World) SetDirectoryEntry(entry any) {
	// DirectoryEntry is deprecated, this method is a no-op for now
	_ = entry
}

// GetDirectoryEntry retrieves the stored DirectoryEntry
func (w *World) GetDirectoryEntry() any {
	w.mu.RLock()
	defer w.mu.RUnlock()
	// DirectoryEntry is deprecated, return nil
	return nil
}

// SetFileInfoList stores a list of FileInfo for testing
func (w *World) SetFileInfoList(fileInfos any) {
	w.mu.Lock()
	defer w.mu.Unlock()
	if list, ok := fileInfos.([]novuspack.FileInfo); ok {
		w.fileInfoList = list
	}
}

// GetFileInfoList retrieves the stored FileInfo list
func (w *World) GetFileInfoList() []novuspack.FileInfo {
	w.mu.RLock()
	defer w.mu.RUnlock()
	return w.fileInfoList
}
