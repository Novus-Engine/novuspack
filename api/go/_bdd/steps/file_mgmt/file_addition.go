...
// WorldInterface defines the methods available on the World object
...
func getWorldTyped(ctx context.Context) WorldInterface {
	...
	if typed, ok := w.(WorldInterface); ok {
		return typed
	}
	return nil
}
...
func fileIsAdded(ctx context.Context) error {
	...
	// Add a test file using StageFile
	testData := []byte("test file content for BDD")
	err := pkg.(interface {
		StageFile(context.Context, string, []byte, interface{}) error
	}).StageFile(world.NewContext(), "test_file.txt", testData, nil)
	if err != nil {
		world.SetError(err)
		return err
	}

	// Store file info for verification
	world.SetPackageMetadata("last_added_file", "test_file.txt")
	world.SetPackageMetadata("last_added_data", testData)
